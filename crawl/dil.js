const {collect_urls} = require("./url_collector.js");//获取url
const {collect_responses} = require("./response_collector");//获取响应
...
const save = async () => {
    try {
        ...
        await execute_query(client, query);//执行sql语句
        await client.end();
    } catch (e) {
...
(async () => {
    site = process.argv[2];//站点
    rank = parseInt(process.argv[3]);//排名
    let ignoreHTTPSErrors = Boolean(process.argv[4]);//是否为https
    let login = Boolean(process.argv[8]); //模块是否为login
